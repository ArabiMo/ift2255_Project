...
public class CourseController {
  ...
  public void getCourseById(Context ctx) {
    ...
    if (!validateCourseId(id)) {
      ctx
        .status(400)
        .json(ResponseUtil.formatError("Le paramètre id n'est pas valide."));
      return;
    }
    ...
    if (course.isPresent()) {
      ...
      ctx
        .status(404)
        .json(
          ResponseUtil.formatError("Aucun cours ne correspond à l'ID: " + id)
        );
    }
  }
  ...
  public void getPrerequisitesCourseById(Context ctx) {
    String id = ctx.pathParam("id");
    ...
    try {
      List<Course> prereqs = service.getPrerequisitesCoursesById(id);
      ctx.json(prereqs); // can be empty list []
